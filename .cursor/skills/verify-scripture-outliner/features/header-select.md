...
- **Section with summary.** Select `data-word-id` 0 then 8, choose **Section**, then **Summary** `The LORD is shepherd`. Run `page.locator('[data-word-id="0"]').click()`, `page.locator('[data-word-id="8"]').click()`, `page.getByTestId("action-section").click()`, fill and save summary. One `section-header` contains that text. Stored `segments[0]` is `{ start: 0, end: 8 }`.
