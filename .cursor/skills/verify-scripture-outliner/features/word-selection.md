...
- **Single word.** Tap the first word. Run `page.getByTestId("word").first().click()` or `page.locator('[data-word-id="0"]').click()`. Exactly one `.selected` word. `pin-start` and `pin-end` are visible (`hidden` attribute absent). `selection-toolbar` is visible with Section / Deeper / Shallower / Summary. Sticky `action-bar` shows Clear / Delete only. `selection-hint` is hidden.
