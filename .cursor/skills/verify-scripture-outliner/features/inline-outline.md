...
- `placeholder-header` keeps an empty summary visible as a bold snippet of the range.
...
- Optional: **Summary** to replace placeholder snippets with typed headings.
...
- **Section header.** Select `data-word-id` 0 then 24, choose **Section**, then **Summary** `The LORD is shepherd`. Run `page.locator('[data-word-id="0"]').click()`, `page.locator('[data-word-id="24"]').click()`, `page.getByTestId("action-section").click()`, fill and save summary. One `section-header[data-depth="0"]` is visible in `passage`, bold, above the body words.
