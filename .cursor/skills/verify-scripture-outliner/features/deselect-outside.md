...
- `deselect-keeps-segments` marks a range with Deeper, selects it, taps empty space, and leaves the header in the passage.
...
- **Keep segments.** Select words `0`–`6`, choose **Deeper**, then **Summary** so a header exists. Select word `0` again, tap wrap padding. Header count is unchanged. Zero `.selected` words.
